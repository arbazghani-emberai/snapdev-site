...
type Selection = { kind: "live" } | { kind: "log"; id: string } | null;
...
function InboxPageInner() {
  ...
  const [selection, setSelection] = useState<Selection>(session ? { kind: "live" } : null);
  const [scheduleEngineer, setScheduleEngineer] = useState<Engineer | null>(null);
  const [connectOpen, setConnectOpen] = useState(false);
  const projectCountAtOpenRef = useRef(0);

  // Historical log composer (kicks off a brand-new live session from a past thread).
  const [logDraft, setLogDraft] = useState("");
  const [logListening, setLogListening] = useState(false);
  const [logAttachMenuOpen, setLogAttachMenuOpen] = useState(false);
  const [logSnippetMode, setLogSnippetMode] = useState(false);
  const [logSnippetDraft, setLogSnippetDraft] = useState("");
  const logAttachMenuRef = useRef<HTMLDivElement>(null);
  const logFileInputRef = useRef<HTMLInputElement>(null);

  // Live composer (continues the active session).
  ...
  useClickOutside(logAttachMenuRef, logAttachMenuOpen, () => {
    setLogAttachMenuOpen(false);
    setLogSnippetMode(false);
    setLogSnippetDraft("");
  });
  ...
  // Whenever a new session starts (guest bootstrap, "Get unstuck", "Start
  // session", or the log composer below), jump the detail pane to it and
  // show a brief "connecting" beat - reset during render, not an effect,
  ...
  }, [session?.messages.length, selection]);
  ...
  const activeLog = selection?.kind === "log" ? (filteredLog.find((s) => s.id === selection.id) ?? SESSION_LOG.find((s) => s.id === selection.id)) : null;
  const showLive = selection?.kind === "live" && !!session;

  // Reset the historical composer's draft whenever the selected log changes.
  const [prevLogId, setPrevLogId] = useState(activeLog?.id);
  if (activeLog?.id !== prevLogId) {
    setPrevLogId(activeLog?.id);
    setLogDraft("");
  }
  ...
  const toggleLogMic = () => {
    ...
    setTimeout(() => setLogListening(false), 1600);
  };
  const openLogFilePicker = (kind: "file" | "image") => {
    const input = logFileInputRef.current;
    if (input) {
  ...
  const handleLogFileSelected = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) setLogDraft((d) => (d ? `${d} ` : "") + `📎 ${file.name}`);
    e.target.value = "";
  };
  const submitLogSnippet = (e: React.FormEvent) => {
    e.preventDefault();
    const value = logSnippetDraft.trim();
    if (!value) return;
    setLogDraft((d) => (d ? `${d}\n\n` : "") + "```\n" + value + "\n```");
    setLogSnippetDraft("");
    setLogAttachMenuOpen(false);
  ...
  return (
    <div className="bg-surface fixed inset-x-0 bottom-0 top-16 grid grid-cols-1 overflow-hidden md:grid-cols-[320px_1fr]">
      {/* Conversation list */}
      <div
        className={`border-line-2 flex-col border-b md:flex md:border-r md:border-b-0 ${selection ? "hidden md:flex" : "flex"}`}
      >
        <div className="p-4">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          {session && (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-sm p-3 text-left transition ${
                selection?.kind === "live" ? "bg-brand-wash" : "hover:bg-surface-2"
              }`}
            >
          ...
          {session && filteredLog.length > 0 && (
            <div className="text-ink-3 px-3 pt-3 pb-1.5 text-[10px] font-bold tracking-[0.08em] uppercase">Past</div>
          )}

          {filteredLog.length === 0 && !session ? (
            ...
            filteredLog.map((s) => (
              <button
                key={s.id}
                type="button"
                onClick={() => setSelection({ kind: "log", id: s.id })}
                className={`flex w-full items-center gap-3 rounded-sm p-3 text-left transition ${
                  selection?.kind === "log" && selection.id === s.id ? "bg-brand-wash" : "hover:bg-surface-2"
                }`}
              >
      ...
      {!selection ? (
        ...
        <div className="grid grid-cols-1 overflow-hidden lg:grid-cols-[1fr_300px]">
          <div className="flex flex-col overflow-hidden">
            <div className="border-line-2 flex items-center gap-3 border-b p-4">
              ...
              <Avatar engineer={activeLog.engineer} size={36} />
              <div className="min-w-0 flex-1">
                <div className="truncate text-[14px] font-semibold">{activeLog.topic}</div>
                <div className="text-ink-2 truncate text-[12px]">
                  with {activeLog.engineer.name} · {activeLog.date} · {activeLog.duration}
                </div>
              </div>
              <button
                type="button"
                onClick={() => setScheduleEngineer(activeLog.engineer)}
                ...
                {activeLog.transcript.map((m, i) =>
                  m.role === "you" ? (
                    ...
                    <div key={i} className="flex items-start gap-2.5">
                      <Avatar engineer={activeLog.engineer} size={28} />
                      <div className="border-line bg-surface max-w-[75%] rounded-md border px-4 py-2.5 text-[14px] leading-relaxed">
                        {m.text}
                      </div>
                    </div>
            ...
            <form
              onSubmit={(e) => {
                ...
                startSession(activeLog.engineer, undefined, logDraft.trim());
                setLogDraft("");
              }}
              className="flex justify-center px-4 pt-3 pb-4"
            >
              <div className="relative w-full max-w-[640px]">
                <div ref={logAttachMenuRef} className="absolute top-1/2 left-1.5 -translate-y-1/2">
                  <button
                    type="button"
                    onClick={() => setLogAttachMenuOpen((v) => !v)}
                    aria-label="Attach"
                    className="text-ink-2 hover:bg-surface-2 hover:text-ink grid size-8 place-items-center rounded-full transition"
                  >
                    <Paperclip className="size-4" strokeWidth={1.75} />
                  ...
                  {logAttachMenuOpen && (
                    <div className="animate-menu-in border-line bg-surface absolute bottom-[calc(100%+8px)] left-0 z-20 w-56 origin-bottom-left rounded-md border p-1.5 shadow-xl">
                      {logSnippetMode ? (
                        <form onSubmit={submitLogSnippet} className="flex flex-col gap-2 p-1.5">
                          <textarea
                            autoFocus
                            value={logSnippetDraft}
                            onChange={(e) => setLogSnippetDraft(e.target.value)}
                            rows={4}
                            placeholder="Paste code here..."
                            className="border-line focus:border-brand bg-surface-2 w-full resize-none rounded-lg border p-2.5 font-mono text-[12px] outline-none transition"
                          />
                        ...
                        <>
                          ...
                          <button
                            type="button"
                            onClick={() => openLogFilePicker("file")}
                            className="hover:bg-surface-2 flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-[13.5px] font-medium transition"
                          >
                            <Download className="size-3.5 -rotate-90" strokeWidth={1.75} />
                            Upload a file
                          ...
                          <button
                            type="button"
                            onClick={() => openLogFilePicker("image")}
                            className="hover:bg-surface-2 flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-[13.5px] font-medium transition"
                          >
                            <ImageIcon className="size-3.5" strokeWidth={1.75} />
                            Share a screenshot
                          ...
                          <button
                            type="button"
                            onClick={() => setLogSnippetMode(true)}
                            className="hover:bg-surface-2 flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-[13.5px] font-medium transition"
                          >
                            <Code className="size-3.5" strokeWidth={1.75} />
                            Paste a snippet
                ...
                <input ref={logFileInputRef} type="file" onChange={handleLogFileSelected} className="hidden" />
                ...
                <input
                  ...
                  autoComplete="off"
                  placeholder={`Message ${activeLog.engineer.name}...`}
                  className="border-line focus:border-brand bg-surface w-full rounded-full border py-3.5 pr-14 pl-11 text-[16px] outline-none transition sm:text-[14.5px]"
                />
                ...
                {logDraft.trim() ? (
                  ...
                  <button
                    type="button"
                    onClick={toggleLogMic}
                    aria-label="Voice input"
                    className={`absolute top-1/2 right-1.5 grid size-9 -translate-y-1/2 place-items-center rounded-full transition ${
                      logListening ? "bg-brand text-white" : "text-ink-2 hover:bg-surface-2 hover:text-ink"
                    }`}
                  >
                    <Mic className="size-4" strokeWidth={1.75} />
                  </button>
        ...
        <div className="hidden flex-1 place-items-center p-10 text-center md:grid">
          <div>
            <div className="text-[15px] font-semibold">Conversation ended</div>
            <p className="text-ink-3 mt-1 text-[13px]">Pick one on the left to see how it went.</p>
          </div>
        </div>
      )}
